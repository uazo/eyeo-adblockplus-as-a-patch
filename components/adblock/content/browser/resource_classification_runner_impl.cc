...
FilterMatchResult ResourceClassificationRunnerImpl::ShouldBlockPopup(
  ...
  auto sitekey = sitekey_storage_->FindSiteKeyForAnyUrl({popup_url, opener});

  auto classification_result = resource_classifier_->ClassifyPopup(
      subscription_collections, popup_url, opener,
  ...
  if (result == FilterMatchResult::kBlockRule)
    LOG(INFO) << "[eyeo] !!! Prevented loading of pop-up " << popup_url.spec();
  else
    LOG(INFO) << "[eyeo] Pop-up allowed (exception)";
  for (auto& observer : observers_) {
    observer.OnPopupMatched(popup_url, result, opener, frame_host,
